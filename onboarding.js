...
const PACKAGE_LABELS = {
  'tune-up':   '🔎 The "Tune Up" — $297 one-time',
  'auto-pilot':'🤖 The "Auto Pilot" — $1,000 + $50/mo',
  'overhaul':  '🚀 The "Overhaul" — from $2,997 one-time',
  'not-sure':  '🤔 Not sure yet / just exploring',
};

const PACKAGE_COLORS = {
  'tune-up':   { bg: '#eff6ff', color: '#1d4ed8' },
  'auto-pilot':{ bg: '#f0fdf4', color: '#15803d' },
  'overhaul':  { bg: '#ecfccb', color: '#14532d' },
  'not-sure':  { bg: '#f8fafc', color: '#475569' },
};
...
<table width="100%" cellpadding="0" cellspacing="0" style="max-width:620px;margin:0 auto;background:#f8fafc;border-radius:16px;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,.08);">
  ...
  <tr><td style="padding:28px 32px;">
    ...
    ${section('Goals & Package Interest', '🎯', `
      ${d.packageInterest ? (() => {
        const pc = PACKAGE_COLORS[d.packageInterest] || { bg: '#f1f5f9', color: '#475569' };
        return row('Package interested in', `<span style="display:inline-block;background:${pc.bg};color:${pc.color};font-size:13px;font-weight:700;padding:5px 14px;border-radius:100px;">${PACKAGE_LABELS[d.packageInterest] || d.packageInterest}</span>`);
      })() : ''}
...
exports.handler = async function (event) {
  ...
  const packageSuffix = data.packageInterest && data.packageInterest !== 'not-sure'
    ? ` · ${PACKAGE_LABELS[data.packageInterest] || data.packageInterest}`
    : '';
  const subject = `[New Client] ${name} — ${businessName}${packageSuffix}`;
